...
public class ApiTest {
	...
	public void createNewUserPositiveScenario() {
		try {
			...
			String payload = "{\"name\":\"Test User\",\"email\":\"" + generateRandomEmail(20)
					+ "\",\"gender\":\"male\",\"status\":\"active\"}";
			MediaType mediaType = MediaType.parse("application/json");
			RequestBody body = RequestBody.create(mediaType, payload);
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/users").method("POST", body)
					.addHeader("Authorization", "Bearer " + apiToken).addHeader("Content-Type", "application/json")
					.build();
	...
	public void createNewPostPositiveScenario() {
		try {
			...
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/posts").method("POST", body)
					.addHeader("Authorization", "Bearer " + apiToken).addHeader("Content-Type", "application/json")
					.build();
			Response response = client.newCall(request).execute();
			if (response.code() == 201) {
	...
	public void createNewCommentPositiveScenario() {
		try {
			...
			String payload = "{ \"post_id\": \"18695\", \"name\": \"Asha Asan\", \"email\": \""
					+ generateRandomEmail(20)
					+ "\", \"body\": \"Dolorem doloribus voluptatum. Ipsa tempore aut. Id perferendis doloribus.\" }";

			MediaType mediaType = MediaType.parse("application/json");
			RequestBody body = RequestBody.create(mediaType, payload);
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/comments").method("POST", body)
					.addHeader("Authorization",
							"Bearer 8e82afb32e5acd2f1ffcd5f5229259c91ddd1f312ae3f363bc7fc3e814947eab")
	...
	public void createtodoPositiveScenario() {
		try {
			...
			RequestBody body = RequestBody.create(mediaType, "{\"user_id\":" + userId
					+ ",\"title\":\"Administratio nihil vomica dedecor viduo auctor est.\",\"due_on\":\"2023-05-31T00:00:00.000+05:30\",\"status\":\"pending\"}");
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/todos").method("POST", body)
					.addHeader("Authorization",
							"Bearer 8e82afb32e5acd2f1ffcd5f5229259c91ddd1f312ae3f363bc7fc3e814947eab")
					.addHeader("Content-Type", "application/json").build();
			Response response = client.newCall(request).execute();
	...
	public void createNewUserNegativeScenarioSameEmail() {
		...
		try {
			...
			MediaType mediaType = MediaType.parse("application/json");
			RequestBody body = RequestBody.create(mediaType,
					"{\"name\":\"Tanushrhhi Siddngh\",\"email\":\"singh_tanuvvvshri@ddjast-schiller.test\",\"gender\":\"male\",\"status\":\"active\"}");
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/users").method("POST", body)
					.addHeader("Authorization", "Bearer " + apiToken).addHeader("Content-Type", "application/json")
					.build();
			Response response = client.newCall(request).execute();
			if (response.code() == 422) {
				...
				System.out
						.println("Create New User API Negative Scenario Negative Scenario with already used email id");
				System.out.println("Response Code: " + response.code());
	...
	public void createNewUserNegativeScenarioInvalidEmailFormat() {
		...
		try {
			...
			MediaType mediaType = MediaType.parse("application/json");
			RequestBody body = RequestBody.create(mediaType,
					"{\"name\":\"Tanushrhhi Siddngh\",\"email\":\"singh_tanuvvvshst-schiller.test\",\"gender\":\"male\",\"status\":\"active\"}");
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/users").method("POST", body)
					.addHeader("Authorization", "Bearer " + apiToken).addHeader("Content-Type", "application/json")
					.build();
			Response response = client.newCall(request).execute();
	...
	public void createNewUserNegativeScenarioMandatoryFieldMissing() {
		...
		try {
			OkHttpClient client = new OkHttpClient();
			String payload = "{\"email\":\"" + generateRandomEmail(20)
					+ "\",\"gender\":\"male\",\"status\":\"active\"}";
	...
	public void fetchCreatedUserDetailsAndCompare() {
		...
		try {
			...
			Request request = new Request.Builder().url("https://gorest.co.in/public/v2/users/1511570")
					.method("GET", null).addHeader("Authorization", "Bearer " + apiToken).build();
			Response response = client.newCall(request).execute();
			if (response.code() == 200) {
				...
				if (email.contentEquals("4z8e9ei1diy@gmail.com")) {
					...
					System.out.println(
							"email Hardcoded: 4z8e9ei1diy@gmail.com is not matching with email from API: " + email);
				}
